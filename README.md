...
- **全銘柄検索**: レポート上の検索ボックスで銘柄名・コード・セクター・シグナルから絞り込み
- **個別銘柄診断**: `check` コマンドで任意の銘柄(収録外もコード指定で可)の買い目を即座に判定
...
# 個別銘柄の買い目診断(収録外の銘柄も証券コードで指定可能)
python -m kabu_analysis check 7203          # コード指定
python -m kabu_analysis check トヨタ 9984    # 銘柄名(収録銘柄のみ)・複数指定も可
```
